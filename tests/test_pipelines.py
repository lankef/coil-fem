...
def test_elastic_pipeline_uniform_material():
    ...
    # Uniform unit Winkler stiffness (surface fully supported), at quad points
    n_sq       = pipeline.n_surface_quads
    support_k  = jnp.ones(n_sq) * 1e9

    result = pipeline.solve(points, body_force, support_k)
...
def test_support_is_not_coupled():
    ...
    assert Support(k_clamp=1e9).is_coupled is False


def test_support_solve_returns_empty_dict():
    """Support.solve returns empty dict (no-op)."""
    state = Support(k_clamp=1e9).solve({'anything': 1})
    assert state == {}
...
def test_support_compute_weights_uniform():
    """Support with no fixed_clamp_fns returns all-ones grounded weights."""
    support = Support(k_clamp=1e9)
    pts = jnp.ones((7, 3))
    w_g, w_a = support.compute_weights(0, pts, None, None)
    assert w_g.shape == (7,)
    assert w_a.shape == (7,)
    assert jnp.all(w_g == 1.0)
...
def test_support_compute_weights_custom_fn():
    ...
    support = Support(k_clamp=1e9, fixed_clamp_fns=half_fn)
    pts = jnp.ones((6, 3))
    w_g, w_a = support.compute_weights(0, pts, None, None)
    assert w_g.shape == (6,)
    assert jnp.allclose(w_g, 0.5)
    assert jnp.all(w_a == 0.0)
...
def test_coil_fem_has_pipelines_and_support():
    ...
    support = Support(k_clamp=1e9, fixed_clamp_fns=_constant_fixed_clamp_fn)
    fem = CoilFEM(
...
def test_coil_fem_requires_support():
    """CoilFEM requires an explicit support argument."""
    curve = _make_circle(N=4, R=1.0)
...
def test_thermo_elastic_pipeline_solve_raises():
    ...
    with pytest.raises(NotImplementedError, match="not yet implemented"):
        stub.solve(points, body_force, jnp.ones(stub.n_surface_quads))
...
def test_solve_uncoupled_matches_inline():
    ...
    k1 = jnp.ones(p1.n_surface_quads) * 1e9
    k2 = jnp.ones(p2.n_surface_quads) * 1e9

    params = {
        'mesh_points_by_coil': [pts1, pts2],
        'body_force_by_coil':  [bf1, bf2],
        'stiffness_by_coil':   [k1, k2],
    ...
    sol1_direct = p1.solve(pts1, bf1, k1)['sol_list']
    sol2_direct = p2.solve(pts2, bf2, k2)['sol_list']
