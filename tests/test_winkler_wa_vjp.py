"""Diagnostics for grounded-w_a Winkler VJP vs beam-coupling dual use.

Also covers the simsopt-free Taylor JIT mask fix (concrete integer indices).


Localizes the ~1–2% Taylor dJh/FD gap on W7-X SupportBeams:
``k = k_clamp*w_g + k_attachment*w_a`` also drives ``K_cs``/``K_sc``/``K_ss``.

GPU-gated.  Prints numeric tables (also useful as a notes appendix).
See ``notes/WINKLER_WA_VJP.md``.
"""
...
# ---------------------------------------------------------------------------
# Step 2 — ablation A/B + drop-w_a (subprocess: one cuDSS build per process)
# ---------------------------------------------------------------------------

def _phi_grad_once(*, ablation: str = "none", drop_wa: bool = False):
    """Build one CoilFEM and return (dJ_analytic, dJ_fd, J0)."""
    if ablation == "none":
        os.environ.pop(_VJP_ABLATION_ENV, None)
    else:
        os.environ[_VJP_ABLATION_ENV] = ablation
    fem, curves, support_dofs0 = _tiny_support_beams_fem(k_attachment=1e10)
    if drop_wa:
        k_clamp = float(fem.support.k_clamp)
        fem.support.stiffness = lambda w_g, w_a: k_clamp * w_g
    cdofs = [c.dofs for c in curves]
    idofs = jnp.ones(len(curves))
    ...
    phi0 = float(support_dofs0['phis_start_cc'][0][0])
    d_an = float(jax.grad(J_of_phi0)(phi0))
    d_fd = _dir_fd(J_of_phi0, phi0, eps=1e-5)
    return d_an, d_fd, float(J_of_phi0(phi0))


@pytest.mark.skipif(not (_HAS_SPINEAX and _HAS_GPU), reason=_GPU_REASON)
def test_vjp_ablation_phi_grad_subprocess():
    """Run ablation / drop-w_a probes sequentially (one cuDSS build per process).

    Heavy: spawns four GPU processes.  Prefer
    ``python notes/winkler_wa_ablation_probe.py`` when iterating locally.
    """
    import subprocess
    import sys
    import time
    ...
    script = os.path.join(root, "notes", "winkler_wa_ablation_probe.py")
    cases = [
        ("none", []),
        ("freeze_k", ["--ablation", "freeze_k"]),
        ("freeze_sdofs_geom", ["--ablation", "freeze_sdofs_geom"]),
        ("drop_wa", ["--drop-wa"]),
    ]
    ratios = {}
    for name, cli_args in cases:
        env = os.environ.copy()
        env.pop(_VJP_ABLATION_ENV, None)
        ...
        if proc.returncode != 0:
            ...
            # Parent pytest can pin GPU memory; treat OOM as skip not fail.
            if "RESOURCE_EXHAUSTED" in (proc.stderr or "") or "OUT_OF_MEMORY" in (
                proc.stderr or ""
            ):
                pytest.skip(f"GPU OOM during ablation probe {name}")
            pytest.fail(f"probe {name} failed rc={proc.returncode}")
        for line in proc.stdout.splitlines():
            if line.startswith("dJh/FD="):
                ratios[name] = float(line.split("dJh/FD=")[1].split()[0])
        time.sleep(2.0)  # let CUDA tear down before next cuDSS handle

    print("\n=== Ablation ratio summary ===", flush=True)
    for k, v in ratios.items():
        print(f"  {k:20s}  dJh/FD={v:.8f}", flush=True)
    # Baseline / drop_wa must match FD; freeze_* are diagnostic (large on tiny mesh).
    assert abs(ratios["none"] - 1.0) < 0.05, ratios
    assert abs(ratios["drop_wa"] - 1.0) < 0.05, ratios
    # Neither ablation alone recovers FD when both dual w_a paths are large.
    assert abs(ratios["freeze_k"] - 1.0) > 0.5, ratios
    assert abs(ratios["freeze_sdofs_geom"] - 1.0) > 0.5, ratios
